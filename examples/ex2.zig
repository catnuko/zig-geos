...
pub fn main() !void {
    ...
    const steps: u8 = 10;
    const xstep: f64 = (xmax - xmin) / @as(f64, @floatFromInt(steps));
    const ystep: f64 = (ymax - ymin) / @as(f64, @floatFromInt(steps));
    ...
    while (i < steps) : (i += 1) {
        while (j < steps) : (j += 1) {
            ...
            const x = xmin + xstep * @as(f64, @floatFromInt(i));
            const y = ymin + ystep * @as(f64, @floatFromInt(j));
    ...
    const result = c.GEOSGeom_createCollection(c.GEOS_MULTIPOINT, geoms.items.ptr, @as(c_uint, @intCast(geoms.items.len)));
